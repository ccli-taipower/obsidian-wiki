...
| 術語 | 英文 | 回答的問題 | 範例 |
|------|------|----------|------|
| **調號** | key signature | 用了哪些音？（譜號上的升降記號） | 零個升降 → C D E F G A B |
| **調性** | key | 主音是誰 + 大調還是小調？ | C 大調、A 小調 |
| **調式** | mode | 音與音之間的距離模式是什麼？ | Ionian、Dorian、Phrygian… |

調性只區分大/小調，調式則涵蓋七種教會調式——**調性是調式的子集**。日常口語裡兩者常混用，但概念上有這層差別。

> **找調號的速記法：** 升記號 → 最後一個升記號往上半音 = 大調主音；降記號 → 倒數第二個降記號 = 大調主音。

同一個調號可以對應七種調式。例如零個升降（全白鍵），如果主音是 C 就是 Ionian（大調），主音是 D 就是 Dorian，主音是 A 就是 Aeolian（自然小調）。**調號決定音群，主音決定調式。**

> 但調號只能直接告訴你大調的主音。實際的主音（tonal center）要靠聽——旋律不斷回到、結束在哪個音，那個音就是主音。找到主音後，再看它到其他音的距離，就能判斷調式（詳見 §7）。
