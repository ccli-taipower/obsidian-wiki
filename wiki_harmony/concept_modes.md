...
- **調號**（key signature）告訴你「用了哪些音」——看譜號上的升降記號就知道（升記號：最後一個升半音 = 大調主音；降記號：倒數第二個降記號 = 大調主音）
- **調式**（mode）告訴你「這些音的排列方式」——大調、小調、教會調式，都是在描述音與音之間的距離模式（全音/半音的順序）

同一個調號可以對應七種調式。例如零個升降（全白鍵），如果主音是 C 就是大調，主音是 A 就是自然小調，主音是 D 就是 Dorian。**調號決定音群，主音決定調式。**
