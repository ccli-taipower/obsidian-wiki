...
|------|------------|---------|
| 藥物 | Concor 1.25 mg | ✅ 同 |
| 起步劑量 | 1.25 mg QD | **1.25 mg QOD（更保守，平均 0.54 mg/日）** |
| 服用時機 | 非透析日早上 | ✅ 二/四/六早上（非透析日 = T/Th/Sat）|
| 透析時血藥濃度 | < peak | **36-60 hr 後 < 12% peak**（透析時幾乎無 BB 效應 → 不加重 IDH） |
| 滴定上調 | 2-4 週後 → 2.5 mg QD → 5 mg QD | Phase 1（QOD）→ Phase 2（QD）→ Phase 3（2.5 mg）→ Phase 4（5 mg）|
| 穩定後替代 | 可考慮學名藥（倍適能 / 畢舒適 / 百穩，多為 5 mg）| 待 Phase 4 後考慮 |

**為何 QOD 比 QD 更安全（給 IDH-prone 病人）：**
- Bisoprolol 半衰期 10-12 hr，QOD 服用第二天血藥濃度剩 ~25%，第三天剩 ~6%
- 「服用日 → 第二天 washout → 再服用」這個 pattern 等同 *N=1 trial-on*，每次服用都是測試容忍度
- 平均日劑量 0.54 mg（QD 的 43%），對 IDH 影響最小化
- 心臟保護效益仍存在（peak BB 效應在服藥日 2-4 hr），但避開了透析時段
...
### A. 缺鐵性貧血（**2026/06/04 已明確化**）
- **Hb 12.3 → 11.2**（−1.1，需處理）
- **Ferritin 35.3、TSAT 7.0%**（明確功能性缺鐵）
...
**Trajectory**：3/19: 5.7 → 4/9: 6.0 → 5/9: 5.4 → 6/4: 5.0 → **7/4: 6.6 HH** 🚨 → **7/23: 5.0** ✅ → **7/29: 5.3** ✅ → **8/6: 4.5** ✅ → **9/10: 5.3** ✅（稍升但仍 normal、Kalimate taper 建議暫緩觀察一次）
