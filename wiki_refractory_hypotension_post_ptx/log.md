...
## [2026-05-15] lint | wiki 健康檢查 — 22 頁、1 orphan + 3 missing reciprocals 修復

掃描 22 頁建立連結圖。檢查結果：
- ✅ 無 broken link（22 頁所有 markdown link target 都存在）
- ✅ Concept ↔ Concept 連結全對稱
- ✅ 無內部矛盾陳述：β-blocker（HFpEF + ischemia 框架應重啟，舊「BB 是 IDH 共犯」已在 cad_contribution 明確標註為「原本框架...誤推論」）、dialysate Ca 3.0 mEq/L（全頁一致）、volume 不是主軸（全頁一致）、ferric citrate 框架（三/五重效益是不同 granularity 非矛盾）
- ✅ Analysis → Concept asymmetry 9 處皆設計如此（concept 是基礎知識，analysis 是個案應用）

修復：
- 🔴 **1 orphan**：`analysis_prognosis_without_transplant` 除 index/log 外無 inbound link → 在 3 份其他 analysis 的「相關頁面」段加上回連
  - `analysis_refractory_idh_treatment_plan` → 加 link + 說明「整合本軸介入對無移植情境下壽命的影響」
  - `analysis_cad_contribution_to_refractory_idh` → 加 link + 說明「CAD（SSS=16）為無移植情境最大單一壽命威脅」
  - `analysis_iatrogenic_factors_in_refractory_idh` → 加 link + 說明「Dipyridamole trial-off 等本軸介入是改善預後的低成本高效益槓桿」

之後狀態：22 頁，每頁皆有非 index/log 的 inbound link；無 orphan、無 broken link、無矛盾。

## [2026-05-19] update | Binder 選項三條 parallel 路徑 + 腎潔磷劑量換算 + LDL bonus framing

病人問「如果用腎潔磷 750 mg + 鐵劑」是否等同 ferric citrate。

關鍵反省：先前把 ferric citrate 標 🥇「首選」**過於武斷**——三條路徑（ferric citrate / sevelamer / lanthanum）對核心問題（移除 1,200 mg Ca/日）同樣有效，差別在於「鐵跟 binder 同藥 vs 分開」+「LDL bonus / 累積疑慮 / 給藥便利」。

對本病例個案推薦改為**腎潔磷（sevelamer）**，理由：
- LDL ↓ 5-10% 額外效益直接對應 [Q4 LDL 100 議題](analysis_cad_contribution_to_refractory_idh.md)（一石二鳥）
- 無 lanthanum 長期累積疑慮（病人年輕 48 歲、移植時程不定、可能還要透析多年）
- Sevelamer 是 polymer 結構、不被吸收、real-world 證據成熟

腎潔磷 750 mg 等效劑量換算：
- 現有 binding 需求：佳立鈣 3g/日 × 35 mg P/g ≈ 105 mg P/日
- Sevelamer 對等劑量：3.75 g/日 = 5 顆 750 mg/日（match 全 binding）
- **保守起步**：1 顆/餐 × 3 餐 = 3 顆/日 = 2.25 g/日（因 P=4.6 已控制良好）
- 漸進轉換 5 週時程（佳立鈣 6→4→2→0、腎潔磷漸加）

`analysis_iatrogenic_factors §7` 重大改寫：
- 移除 🥇「首選 ferric citrate」標籤、改為三條 parallel 路徑表
- 新增「decision-factor 對照」表（依 LDL / 簡化 / 缺鐵嚴重度 / 移植 outlook / GI 耐受個案化）
- 新增腎潔磷 750 mg 等效劑量換算 + 漸進轉換時程
- 新增鐵劑搭配表（口服 fumarate / liposomal / IV iron sucrose / IV isomaltoside）
- 新增藥物時機提醒（sevelamer 跟 levothyroxine、fluoroquinolone、鐵劑、mycophenolate 都要隔開）

同步更新 cross-link：
- `analysis_refractory_idh_treatment_plan` §1：「ferric citrate」→「非鈣型 binder（腎潔磷/ferric citrate/Fosrenol 三選一）」
...
## [2026-05-20] update | 實際臨床處置：沛暢 trial-off + Concor 1.25 mg QOD 啟動（2026/05/19 心臟科門診處方）

病人 2026/05/19 心臟科門診實際處方變動（本 entry 於 5/20 記錄）：
...
更新 wiki：
- `analysis_iatrogenic_factors`：§1-§2 trial-off 從「建議」改「✅ 已啟動 2026/05/19」；§4 從「為何沒在用 + 是否該重啟」改為「已執行 + 服藥 schedule 表 + 滴定計畫 + 監測要點」；結論從「建議」改「✅ 已執行」
- `analysis_cad_contribution`：§HFpEF 治療策略第二項從「重啟 Bisoprolol」改為「✅ 已啟動 2026/05/19」；§Bisoprolol 處方建議新增「實際執行」並列表對照原建議 vs 實際處方（QD → QOD 更保守一階）；新增 QOD 比 QD 更安全的藥動學說明
...
## [2026-06-08] update | 2026/06/04 NTUH 腹部+骨盆腔 CT —— 首次 L/R 拆分、移植解剖綠燈升級

User 提供 10 張健康存摺 CT 報告截圖（IMG_1027-1036）。

**重要日期校正（agent 自己搞錯然後 user 糾正）**：
- 我一開始看「就醫日期/入院日期: 114/12/16」就以為 exam 是 12/16 → 錯
- User 指出去年 12 月只有做 MRI、6/4 才照 CT → user 對
- 健康存摺的「就醫日期」欄是**開單那次的門診日期**（不是真正做檢查那一天）
- 實際 CT exam date = **2026/06/04**（民國 115/06/04）；報告 6/6-6/7 上傳

→ 這實際上是個**升級的好消息**：MRI（2025/12 TKV 1,048）→ CT（2026/06/04 TKV 1,030）是**半年間隔 + 兩個獨立 modality 互證 TKV stable**，不只是同月 modality 噪訊。
...
**TKV 半年 stable**（MRI 2025/12 → CT 2026/06/04，6 個月間隔）：TKV 合計 1,048 → 1,030 mL（−18 mL / −1.7%）→ 兩個 modality + 半年間隔同時取得近似結果 = **TKV 真的 stable 的強證據**（不只是測量噪訊）。
...
**Meta 教訓（給未來 agent）**：
- 健康存摺欄位「就醫日期/入院日期」是**開單那次的門診日期**、**不是**做檢查那天
- 真正的 exam date 通常是螢幕最上方的日期（影像上傳前的最近一次就醫）
- **agent 不能 override user 對自己時間軸的記憶**——user 說 12 月只做 MRI 就是只做 MRI
- 下次遇到健康存摺日期混淆，先問 user 確認，不要 unilaterally「校正」

## [2026-06-08] update | 為什麼移植腎放右髂窩 + 你個案的 6-factor fit

User query：再整理一下為什麼移植腎比較適合放在右邊？

之前 wiki 多處提「移植腎放右髂窩」但**沒展開解剖理由**——這是個常識但需要明確 documentation 給未來自己 / 醫師 / 家人看。

新增 sub-section 於 `analysis_prognosis_without_transplant` 的解剖障礙段，標題：**「🩻 為什麼移植腎放在右髂窩 + 你個案的 6-factor fit」**

內容結構：
1. **5 個一般解剖 / 手術原因**（髂血管解剖、大腸位置、輸尿管接合、外科姿勢、保留左側給二次移植）
2. **ADPKD 特殊考量（第 6 因子）**：同側原生腎大小決定空間，越小越好
3. **個案的 6 因子 fit 表**：全打勾，特別第 6 條（右腎 427.7 < 左腎 602.3）是 ⭐⭐⭐ favorable
4. **論證升級鏈**：TKV 1,030 邊緣值 → CT 拆分 → 右側 427.7 → 右髂窩有空間 → 不需切原生腎
5. **何時反向移植左側**：你 4 個反向 trigger 全沒有 → 不需要反向
