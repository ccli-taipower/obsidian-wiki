...
> 引用方：[[concept_hypermeter]], [[concept_phrase_elision]], [[../wiki_articulation/concept_articulation_in_polyphony]]（內聲部 voicing）
...
對 score-claude 的對應：[[concept_phrase_elision]] 處理 — 但 Brahms 的 elision 比 Schumann / Mozart 更密集 + 更複雜。
...
對 score-claude DP 的意涵：Brahms 段不適合 4-bar fallback（[[concept_hypermeter]] §Brahms）。需更靈活 phrase length 偵測。
...
→ Brahms 主要曲目在 advanced 範圍 — 對 score-claude DP 影響有限（[[../score-claude/memory/project_target_repertoire_intermediate]]）。
...
對 phrase 偵測的意涵：Brahms 主題「**並非完全重複**」— [[concept_subject_imitation_detection]] 對 Brahms 適用度比 Bach 低（subject 沒精確重複）。
...
- Inner voice 對位 → [[../wiki_articulation/concept_articulation_in_polyphony]] 對應

未來如要擴展到 advanced 曲目，Brahms 是好的 stress test 對象。

## 8. 與其他 wiki 頁面的關係

- [[concept_hypermeter]] — Brahms 是不規則 hypermeter 的代表
- [[concept_phrase_elision]] — Brahms elision 大量使用
- [[concept_subject_imitation_detection]] — Brahms developing variation 對其適用度低
- [[../wiki_articulation/concept_articulation_in_polyphony]] — Brahms 對位 + homophonic 混合 texture
- [[../wiki_articulation/concept_tenuto]] — Brahms 內聲部 tenuto 大量使用
- [[composer_chopin_phrasing]] / [[composer_schumann_phrasing]] — 三大浪漫派對比
