...
**接縫延伸（同日第二段，user 核准設計）**：Inv12 音型轉折的主力病灶（RH 16 / LH 12 對）
在 **arpeggio-merge 群組接縫**——上/下交替的分解和弦被 Step-1b 合併成 chord 群組後：
(a) 接縫邊緣同指（如上組頂 A5(5) → 下組頂 F#5(5)）走 chord→chord transition，anchor
計價對它全盲 → **edge-adjacency 計價**：merged 群組本質是序列音，前組時間最後音與
後組時間最先音相鄰，同指且 >2 半音 → 收同一套重定位價（REPEAT+REPOSITION+weak）；
(b) **bottom-to-bottom 接縫的 1→1 被「拇指固定最低音」硬約束強制**（desc 組尾=底音=1、
asc 組頭=底音=1），計價無用 → **接縫相鄰的 merged 群組放寬枚舉**（允許非拇指底音組合
如 2-3-5；user 2026-07-16 選項：只放寬接縫相鄰者——獨立長琶音保持拇指根音教學正解，
真和弦不受影響）。結果：Inv12 接縫重定位 RH 16→7（≤ 已接受樂章天花板 8）、**LH 12→0**、
Inv13 LH 9→1；收割 Inv3-L m24（5→5 大跳 → 4→5）、Inv4-L m42（1→1 → relay）。
