...
## [2026-07-16] standard scale fingering | D♭ RH 書驗證：swing v1 證偽 → v2 深度上限版

user 帶書驗證：D♭ RH 頂音 = **2**（1 2 1），swing v1 尖預測 4（1 4 1）**證偽**。關鍵：G♭ 與 D♭ 迴轉局部幾何完全相同（拇指在白鍵導音、頂音=上方半音黑鍵、n-2 距頂音 3 半音），唯一差別 = n-2 指是 3 還是 4 → 最小修訂 = **swing 深度上限**：頂音指 = min-cost{swing（成本隨 over-cross 深度遞增 1→3 < 1→4）, 重定位（thumb-under commit 用 2 折返即撤，固定成本）}；n-2=3 → swing 贏（G♭ 1 3 1）、n-2=4 → 重定位贏（D♭ 1 2 1）。與 LH a+b 共用同一條跨越深度單調性。**Meta 規則降級**：「迴轉點不為單一音重定位手位」從禁令降為成本項。連帶：(1) 站方 C#/D♭ RH 下行欄 `4 3 2 1` 開頭的支持證據撤回（書版應為 `2 1 4 3...` 開頭，疑站≠書錯誤待修）；(2) DP 表達力反轉——D♭ 選 2 是 pairwise DP 天然行為，超出 pairwise 的只剩 G♭ 選 3 一個 case。⏳ G♭ 1 3 1 待書面二確認（現為 swing 唯一支撐；若也是 2 → 規則坍縮為「RH 頂音一律 2」與 LH 全對稱）。

## [2026-07-14b] standard scale fingering | 頂音邊界規則 RH 分支（swing finger）＋統一 meta 規則

user 指出書版 G♭ **RH** 上行迴轉 = 1 3 1 非 1 2 1，看似與 a+b（LH 2→1 最省）矛盾——實為結構差異：LH 拇指之上必跨越（變數=深度→最淺 2）；RH 到頂音零跨越但用 2 = 為單一音 commit 一次 thumb-under 手位、下一拍立即撤銷（手掌張開再收回=白做一個功），用 3 = 拇指借過去彈 F、手框全程停在 E♭-3 原位，3 盪上 G♭ 再盪回。**統一 meta 規則：迴轉點不為單一音重定位手位**。Swing 規則：RH 次頂音=拇指錨點時，頂音=彈 n-2 音的那根指（非「黑鍵愛 3」）。觸發範圍 RH 只有 D♭/G♭（導音 C/F=RH 拇指錨點）。**尖預測待書驗證：RH D♭ 頂音 = 4（1 4 1）**（n-2=B♭ 在 4 上；站方 C# RH 下行欄 `4 3 2 1` 開頭吻合）。全等宣稱加註（僅 LH；RH 邊界分道）。DP：LH 分支自然湧現；RH commit-revoke 是三音窗二階脈絡超出 pairwise Markov，但 span 區間制 (1,2) 半音壓縮 > (1,3) 或已偏向 3，待 harness 驗。

## [2026-07-14] standard scale fingering | 頂音邊界規則（a+b 推導，「恰好打對」撤回）

user 提出頂音 2 的可推導邊界規則：(a) 頂音後必折返；(b) 折返第一音按錨點是拇指 → 頂音用 2（thumb-under 從 2 底下穿 = CMC 屈曲角度最小，2→1 << 3→1/4→1）。驗證：條件 (b) 在五個黑鍵主音調全部成立（頂音下方音 = 拇指錨點 A/D/G/C/F），且解釋了為何恰好這五調需要例外（黑鍵主音 → 頂音≠1 → 循環推 3/4 → 邊界規則覆蓋）；白鍵調頂音=1 規則不觸發。雙向：到達側自動是 1→2 最淺 cross-over。**定位修訂**：哈農 = 兩層規則系統（循環＋邊界），非「自相矛盾、恰好打對」——2026-07-04 雙軸措辭撤回該句；自洽軸統一版仍勝但差距縮小為一層 vs 兩層。DP 不需顯式規則（帶前瞻的全局最小化 + 穿越成本隨出發指遞增即自然湧現；B♭ LH DP 錨點還錯但頂音已自選 2）。新節 §頂音邊界規則。

user 對照實體哈農書：站方降記號調 LH 上行頂音 B♭/E♭/A♭/D♭=4、G♭=3 全錯。書版 = **五調頂音全部 2**（哈農在邊界放棄循環理論 3/3/3/3/4，改「拇指隔壁 2 指輕收」，與下行起始迴轉無縫）；C/F 頂音 1 不受影響。初版校正曾誤記 D♭=3/G♭=4（循環推論非書版）。推論：**書版哈農與統一版在 D♭/G♭ 逐音全等**；兩版實質差異只剩 B♭/E♭/A♭ 中段錨點——正是哈農贏的地方。DP 評比定稿：統一版無任何調在生物力學上勝出（總帳哈農 677.2 vs 統一 760.0）；其價值在認知層（一條規則＋雙手同步穿越）。
