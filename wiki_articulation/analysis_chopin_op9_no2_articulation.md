...
> 引用方：[[../wiki_phrase/analysis_chopin_op9_no2_nocturne]] §7 fioritura, [[concept_legato_substitution]] §5（Op.9-2 為「適用情境」代表）, [[concept_articulation_overview]] §3 Romantic default, [[concept_period_defaults]] §4 Romantic

## 1. 為什麼挑 Op.9 No.2 作為 Chopin articulation 案例

理由：
- Op.9 No.2 (Eb major Nocturne, 1830-31) 是 Chopin **最知名 + 教學最常用**的 nocturne
- musetrainer/library 提供高品質全曲 MXL（[[../wiki_phrase/analysis_chopin_op9_no2_nocturne]] 已用於 fioritura validation）
- Slur 標記豐富（74 slurs / 148 notes-in-slur），是 score-claude 已啟用 [[concept_legato_substitution]] 的對象
...
- 對 [[concept_legato_substitution]]: m9 + m17 F5 「同音重複 sustained」是經典 substitution 案例（score-claude 已啟用此 rule）
...
- score-claude [[concept_legato_substitution]] §6 + LEGATO_MIN_DURATION gate 正確跳過此段
...
- accent (>) 出現於 sf 強拍
- 對未來 [[concept_staccato]] DP rule 是高度適用對象
...
→ 對應 [[concept_articulation_overview]] §6 提到的「**例外：超長 slur 等同 phrase boundary**」。Op.9-2 部分 long slur 屬此類。
...
對指法的意涵：未來 [[concept_accent_marcato]] DP rule 落地後，這些位置應**強指偏好**（1/2/3）。
...
從 [[../score-claude/memory/project_legato_substitution_v1_2026-05-29]] 與其他 memory：
...
Op.9-2 RH 有 29 個 staccato 標記，主要在 m24 / m29 variation 段。未來 [[concept_staccato]] DP rule 落地後：
- m24 staccato 段：DP 應允許 thumb-cross 較自由、hand-jump penalty 減
- 預期 cost 改善：中等（~5-10 cost units RH）

### 7.2 Accent / sforzando 強指偏好（未實作）

Op.9-2 有 9 個 accent + 2 個 StrongAccent。未來 [[concept_accent_marcato]] rule 落地後：
- 這 11 個音應強指偏好（1/2/3）
...
- [[../wiki_phrase/analysis_chopin_op9_no2_nocturne]] §7 — fioritura 結構分析（本頁是 articulation 補充）
- [[concept_articulation_overview]] §3 — Op.9-2 是 Romantic default 典型案例
- [[concept_legato_substitution]] §5 — Op.9-2 在「適用情境」表中浪漫派 lyrical melody 代表
- [[concept_staccato]] §4 — Op.9-2 m24 staccato 段為未來測試對象
- [[concept_accent_marcato]] §4 — Op.9-2 sf 標記為未來測試對象
- [[concept_period_defaults]] §4 — Chopin 為 Romantic legato default 代言
- [[../score-claude/memory/project_legato_substitution_v1_2026-05-29]] — Op.9-2 legato substitution 啟用 + v2 duration gate
- [[../score-claude/memory/project_fioritura_filter_2026-05-28]] — Op.9-2 fioritura filter 啟用 + §7 校正
