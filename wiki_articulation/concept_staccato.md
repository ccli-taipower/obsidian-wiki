...
Staccato（斷奏）= 每個音獨立 attack，演奏後手指立刻離鍵，相鄰音之間有明顯空隙。

物理意涵：
- 前一音的「釋放」不需要與後一音的「彈下」重疊
- 因此**手位可以在音與音之間自由移動**（不必保持任何手指持按）
- 同音換指（substitution）失去意義 — 沒有「保持 legato 連續性」的需求

→ Staccato 段的指法策略：**鬆綁所有「連續性」相關約束**。
...
| 約束 | 在 legato 段 | 在 staccato 段 |
|---|---|---|
| **Hand jump**（音與音之間手位大幅跳動）| 限制（破壞 legato）| 鬆綁（手位自由移動）|
| **Thumb cross / thumb-pass**（拇指穿越其他指）| 偏好預備、避免突然 cross | 鬆綁（每音獨立，無需預備）|
| **Finger substitution**（同音換指）| 鼓勵（保持 legato）| 無意義（音已釋放）|
| **連續同指**（同手指彈相鄰音）| 強懲罰（audibly broken）| 可接受（快速 staccato 段使用同指反而省動作）|

## 4. 適用情境

| 情境 | 範例 |
|---|---|
| Classical Allegro 主題的「短音」段 | Mozart K545 mvt1 m1-2 RH (open theme, dot-marked) |
| Beethoven scherzando / leggero 段 | Beethoven Op.49 No.2 mvt2 trio 段 |
| Czerny / Hanon 練習曲的 staccato variant | 大量 staccato 段練手指獨立 |
| 浪漫派斷奏 etude | Chopin Op.10 No.4 雖然 advanced 但 staccato 段同樣鬆綁規則 |
